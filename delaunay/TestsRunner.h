...
class TestsRunner
{
	...
	public: 
		...
		IL void make_Setting(Vec3f& v1, Vec3f& v2, Vec3f& v3, Vec3f& v4, Vec3f& r5) {
			r5 = gP, v2 = gP, v3 = gP, v4 = gP;

			while (checkCompl(v1 = gP, v2, v3 , v4 )) {}
		}
		...
		void RunTests(size_t n, tmList test_methods) {
			if (n > 0)
			{
				...
				for (TestMethod test_method : test_methods) {
					...
					const double t1 = getCPUTime();
					for (size_t N = n; --N > 0;) {
						test_result = test_method.RunTest(v1, v2, v3, v4, r5);
					};
					const double t2 = getCPUTime();
					const float test_time = t2 - t1;
					_msg->showResult(
						test_method.getName(), 
						test_result.point_state, test_time, 
						test_result.circumsphere.c);
				}
